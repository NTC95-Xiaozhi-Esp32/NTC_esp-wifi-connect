...
<<<<<<< HEAD
			// 读取并回传SD卡总线配置（存储在同一NVS namespace: "wifi"）
			{
				...
				if (nvs_open("wifi", NVS_READONLY, &nvs) == ESP_OK) {
					int32_t iv = 0;
					if (nvs_get_i32(nvs, "sd_mode", &iv) == ESP_OK) cJSON_AddNumberToObject(json, "sd_mode", iv);

					if (nvs_get_i32(nvs, "sd_clk", &iv) == ESP_OK)  cJSON_AddNumberToObject(json, "sd_clk", iv);
					if (nvs_get_i32(nvs, "sd_cmd", &iv) == ESP_OK)  cJSON_AddNumberToObject(json, "sd_cmd", iv);
					if (nvs_get_i32(nvs, "sd_d0",  &iv) == ESP_OK)  cJSON_AddNumberToObject(json, "sd_d0",  iv);
					if (nvs_get_i32(nvs, "sd_d1",  &iv) == ESP_OK)  cJSON_AddNumberToObject(json, "sd_d1",  iv);
					if (nvs_get_i32(nvs, "sd_d2",  &iv) == ESP_OK)  cJSON_AddNumberToObject(json, "sd_d2",  iv);
					if (nvs_get_i32(nvs, "sd_d3",  &iv) == ESP_OK)  cJSON_AddNumberToObject(json, "sd_d3",  iv);

					if (nvs_get_i32(nvs, "spi_sck",  &iv) == ESP_OK) cJSON_AddNumberToObject(json, "spi_sck",  iv);
					if (nvs_get_i32(nvs, "spi_miso", &iv) == ESP_OK) cJSON_AddNumberToObject(json, "spi_miso", iv);
					if (nvs_get_i32(nvs, "spi_mosi", &iv) == ESP_OK) cJSON_AddNumberToObject(json, "spi_mosi", iv);
					if (nvs_get_i32(nvs, "spi_cs",   &iv) == ESP_OK) cJSON_AddNumberToObject(json, "spi_cs",   iv);
...
<<<<<<< HEAD
			...
			auto set_i32_if_present = [&](const char* json_key, const char* nvs_key, int32_t min_v, int32_t max_v) {
				...
				int32_t val = (int32_t)it->valueint;
				if (val < min_v || val > max_v) return;
				esp_err_t e2 = nvs_set_i32(nvs, nvs_key, val);
				if (e2 != ESP_OK) {
					ESP_LOGE(TAG, "Failed to save %s=%ld to NVS (%s)", nvs_key, (long)val, esp_err_to_name(e2));
				}
			};

			// sd_mode: 0=SDMMC 1-bit, 1=SDMMC 4-bit, 2=SPI
			set_i32_if_present("sd_mode", "sd_mode", 0, 2);

			// SDMMC pins (allow -1 for "not used")
			set_i32_if_present("sd_clk", "sd_clk", -1, 48);
			set_i32_if_present("sd_cmd", "sd_cmd", -1, 48);
			set_i32_if_present("sd_d0",  "sd_d0",  -1, 48);
			set_i32_if_present("sd_d1",  "sd_d1",  -1, 48);
			set_i32_if_present("sd_d2",  "sd_d2",  -1, 48);
			set_i32_if_present("sd_d3",  "sd_d3",  -1, 48);

			// SPI pins (allow -1 for "not used")
			set_i32_if_present("spi_sck",  "spi_sck",  -1, 48);
